...
function opApply(cy, op)
{
	...
	{
		case "AddNode": {
			console.log("AddNode entered:", op)
			cy.add({
				group: "nodes",
				data: { id: op.nodeId, nodeTypeId: op.nodeTypeId, nodeTypeName: op.nodeTypeName, label: op.label ?? op.nodeTypeName, ...op.data },
		...
		case "AddEdge": {
			console.log("AddEdge entered", op)
			cy.add({
				group: "edges",
				data: {
					id: op.edgeId,
...
{
	console.log("Creating graph instance with options:", options);
...
{
	const inst = instances.get(handleId);
	console.log("Applying ops:", ops);
	console.log("nodeId:", ops.nodeId);
	console.log("label:", ops.label);
	if (!inst) return;
